(function () {
    ...
    function showLoadError(message) {
        ...
        content.innerHTML =
            '<header class="doc-page-header"><p class="doc-page-kicker">GIT · WORKFLOW NOTES</p><h1>Git 使用学习</h1></header>' +
            '<div class="load-error" role="alert"><h2>笔记加载失败</h2><p>' + escapeHtml(message) + '</p>' +
            '<a href="./Git使用学习.md">直接查看 Markdown 原文</a></div>';
    ...
    function renderPage(index, targetId) {
        ...
        content.innerHTML =
            '<header class="doc-page-header">' +
                '<p class="doc-page-kicker">GIT · ' + escapeHtml(page.group.plainTitle) + ' · 第 ' + (currentPageIndex + 1) + ' / ' + pages.length + ' 章</p>' +
        ...
        document.title = page.plainTitle + ' · Git 使用学习 · Kyle';
    ...
    function applyTheme(theme, persist) {
        ...
        if (persist) {
            try { window.localStorage.setItem('git-note-theme', normalized); }
            catch (error) {}
        }
    }
