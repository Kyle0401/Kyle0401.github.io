...
一个派生类对象并不是只有“派生类自己的那一部分”。它内部还包含相应的**基类子对象（base class subobject）**。
...
也就是说，销毁 `B` 对象时，会先执行 `B::~B()`，然后再销毁它内部的 `A` 基类子对象并执行 `A::~A()`。
...
> **派生类对象构造：先基类，后派生类；派生类对象析构：先派生类，后基类。**
